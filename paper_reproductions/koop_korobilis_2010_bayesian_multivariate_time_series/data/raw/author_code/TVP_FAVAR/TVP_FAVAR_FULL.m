% TVP-FAVAR Time varying structural factor augmented VAR with stochastic
% volatility
% Based on Korobilis (2009) "Assessing the Transmission of Monetary
% Policy Shocks Using Dynamic Factor Models" 
% ***********************************************************
% The model is:
% 
...
% 
% 
%  with u(t)~N(0,H(t)), and e(t)~N(0,R(t)),  
%
%  where:
%    R(t) = diag{r1(t), .... ,rn(t),0},
%
%    and
%
%    A(t)' x H(t) x A(t) = Sigma(t)*Sigma(t),
%    Sigma(t) = diag{s1(t), .... ,sn(t)},
%             _                                          _
%            |    1         0        0       ...       0  |
%            |  a21(t)      1        0       ...       0  |
%    A(t) =  |  a31(t)     a32(t)    1       ...       0  |
...
% 
% 
% and log(r(t)) = log(r(t-1)) + error,
%     log(s(t)) = log(s(t-1)) + error.
% **************************************************************
%   NOTE:
%      This version does not implement completely the Benchmark model results
%      in my paper, but you can get the results for the competing models.
%      This is because this version does not draw the index 'J' for the
%      volatilities in the factor equation and the lower triangular matrix
%      in the FAVAR equation. Nevertheless, the results are similar, and it
%      is also easy to plug in the routine 'gck.m' for these matrices.
%
%   VERSION: 
%      (Very) first attempt: May 06, 2007
%      This version: May 15, 2008
%   COMPATIBILITY: 
%      Written in MATLAB 2007a for Linux-64 bit. No .mex files involved so it
%      should work in most MATLAB Windows/Linux 32-bit versions. It is also
%      rather easy to translate this code in Gauss or OxMetrics, if you will.
%      29 May 2009: tested on MATLAB 2008b 64-bit with Windows Vista Ultimate
%      and it works fine.
%   AUTHOR: 
%      Dimitris Korobilis
...
%      For corrections about the code please do NOT use my Universtiy e-mail.
%      Instead mail me at: dikorombilis@yahoo.gr
%   ACKNOWLEDGEMENTS: 
%      Some functions (ts_prior,varbase,olssvd,extract,transx,facrot,impulse,gck)
%      were writen (either in MATLAB or GAUSS) by the following guys, whom I
%      thank a lot (in alphabetical order) for having their code available:
%
%      Jean Boivin, Piotr Eliasz, Gary Koop and Mark Watson
%
